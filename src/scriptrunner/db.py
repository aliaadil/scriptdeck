...
MIGRATIONS = {
    ...
    """,
    5: """
    -- Live log viewer (v0.5): add ``status='running'`` for in-flight runs and
    -- ``'cancelled'`` for cancelled runs. SQLite CHECK constraints cannot be
    -- altered in place, so we rebuild the runs table preserving all columns and
    -- data. Foreign keys are temporarily disabled because ``logs.run_id``
    -- references ``runs.id``; they are re-enabled after the rebuild.
