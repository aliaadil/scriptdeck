...
def schedule(conn, script):
    return create_schedule(conn, script["id"], "interval", "5m")
...
def test_evaluate_retry_failure_with_retry_max_zero_marks_exhausted(conn, script) -> None:
    schedule = create_schedule(conn, script["id"], "interval", "5m", retry_max=0)
...
def test_evaluate_retry_retries_until_exhausted(conn, script) -> None:
    schedule = create_schedule(conn, script["id"], "interval", "5m", retry_max=2)
...
def test_schedule_retry_inserts_run_with_same_group_id(conn, script) -> None:
    schedule = create_schedule(conn, script["id"], "interval", "5m", retry_max=3)
    run = _run(conn, script, schedule, status="failure")
    decision = evaluate_retry(conn, run, schedule)
    retry_run = schedule_retry(conn, run=run, schedule=schedule, decision=decision)
...
def test_fire_alert_if_exhausted_returns_false_when_not_exhausted(conn, script) -> None:
    ...
    decision = evaluate_retry(conn, run, schedule)
    assert decision.exhausted is False
    assert fire_alert_if_exhausted(conn, run=run, schedule=schedule, decision=decision) is False
...
def test_fire_alert_only_once_per_retry_cycle(conn, script) -> None:
    ...
    try:
        ...
        retry_run = schedule_retry(conn, run=r1, schedule=schedule, decision=d1)
        assert fire_alert_if_exhausted(conn, run=r1, schedule=schedule, decision=d1) is False
        d2 = evaluate_retry(conn, retry_run, schedule)
        assert d2.exhausted is True
        assert fire_alert_if_exhausted(conn, run=retry_run, schedule=schedule, decision=d2) is True
    finally:
        server.shutdown()
        server.server_close()
...
def test_record_run_result_top_level(conn, script) -> None:
    schedule = create_schedule(conn, script["id"], "interval", "5m", retry_max=1)
    run = _run(conn, script, schedule, status="failure")
    decision, retry_run, _fired = record_run_result(conn, run=run, schedule=schedule)
    assert decision.should_retry is True
...
def test_record_run_result_no_retry_on_success(conn, script, schedule) -> None:
    run = _run(conn, script, schedule, status="success")
    decision, retry_run, fired = record_run_result(conn, run=run, schedule=schedule)
    assert decision.should_retry is False
