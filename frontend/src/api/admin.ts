...
export const listUsers = () => api<Array<{ id: number; email: string; role: string }>>("/users/");
export const createInvite = (email: string, role: "admin" | "editor" | "viewer") =>
  api<{ token: string; expires_at: string }>("/users/invites", {
    method: "POST", body: JSON.stringify({ email, role }),
  });
export const changeRole = (userId: number, role: "admin" | "editor" | "viewer") =>
  api(`/users/${userId}/role`, { method: "PUT", body: JSON.stringify({ role }) });
...
export const listAudit = (params?: { user_id?: number; resource?: string }) => {
  ...
  return api<AuditEntry[]>(`/admin/audit${q.toString() ? `?${q}` : ""}`);
};
